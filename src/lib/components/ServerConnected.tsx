...
export function ServerConnected({ status, fetchStatus }: ServerStatusProps) {
	...
	function handleClick() {
		toast({
			...
			action: status ? undefined : <ToastAction onClick={() => fetchStatus()} altText="Check status">Check status</ToastAction>,
		})
	}
	...
	return (
		<div
			onClick={handleClick}
			className={cn(serverConnectedVariants({ variant: status ? "connected" : "default" }))}
		>
			{status ? <p>✅</p> : <p>❌</p>}
		</div>
	)
